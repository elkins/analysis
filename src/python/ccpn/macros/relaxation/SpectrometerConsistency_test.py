...
__dateModified__ = "$dateModified: 2024-09-06 15:27:27 +0100 (Fri, September 06, 2024) $"
...
def _plotData(dataPath):
    _path = aPath(dataPath)
    filename = _path.stem
    ...
    fig.suptitle(f'Data Consistency Analysis\n{filename}', fontsize=16)
    ...
    for pair in pairs:
        ...
        xs.append(x)
        ys.append(y)
        axs[0, 0].scatter(x, y, label=f'{xSF} MHz vs {ySF} MHz', alpha=0.5, )
    ...
    for pair in pairs:
        ...
        axs[0, 1].scatter(data1_quantiles, data2_quantiles, alpha=0.5)
        axs[0, 1].plot([data1_quantiles.min(), data1_quantiles.max()], [data1_quantiles.min(), data1_quantiles.max()], 'r--')  # Diagonal reference line
        axs[0, 1].set_xlabel(f'Quantiles of {xSF}')
        axs[0, 1].set_ylabel(f'Quantiles of {ySF}')
        axs[0, 1].margins(x=0.35, y=0.35)  # 35% margin on both axes

        axs[0, 1].set_title(f'Q-Q Plot of {xSF} vs {ySF}')
        for i, txt in enumerate(resCodes):
                axs[0, 1].annotate(str(txt), (data1_quantiles[i], data2_quantiles[i]))
    ...
    # Q-score
    percentileValue = 90
    qdata = q_scores_standardized
    percentileData = np.percentile(qdata, percentileValue)

    axs[1, 1].plot(resCodes, qdata, 'o', color='blue')
    axs[1, 1].axhline(y=percentileData, color='red', linestyle='--', linewidth=0.5,  label=f'f{percentileValue}th percentile')
...
dataPath_gb1 = '/Users/luca/Projects/AnalysisV3/src/python/ccpn/AnalysisDynamics/lib/modelFreeAnalysis/examples/gb1/inputs/GB1_rates.xlsx'
dataPath_15445 = '/Users/luca/Documents/NMR-Data/Relaxation/bmrb/15445/15445.xlsx'
dataPath = dataPath_gb1
_plotData(dataPath)
